...
jQuery('#btn_submit_admin').click(function(){
		...
		var idx = jQuery('#aksi_admin_form').val();
				 ...
				 if(idx == 'ubah'){
				   var post_form = "nama="+username+"&pekerjaan="+pekerjaan+"&email="+email+"&nama_tempat="+jQuery('#nama_tempat').val()+"&alamat_donor="+jQuery('#alamat_donor').val()+"&id_admin="+jQuery('#id_admin_input').val()+"&act=ubah"+"&status="+jQuery('#cmb_status').val()+"&alamat="+jQuery('#t_alamat').val()+"&kontak="+jQuery('#kontak_input').val();
				 }
				 else{
					var post_form = "nama="+username+"&pekerjaan="+pekerjaan+"&nama_tempat="+jQuery('#nama_tempat').val()+"&alamat_donor="+jQuery('#alamat_donor').val()+"&email="+email+"&id_admin="+jQuery('#id_admin_input').val()+"&act="+"&status="+jQuery('#cmb_status').val()+"&alamat="+jQuery('#t_alamat').val()+"&kontak="+jQuery('#kontak_input').val();
				 }
				  //alert(post_form);
				  //alert(serial);
				  ...
				  if(username == ""){
						form_admin2.nama_input.focus();	
						
						return false;
				  }
					
				  if(pekerjaan == ""){
						form_admin2.pekerjaan_input.focus();	
						return false;
				  }
				  ...
				  if(alamat == ""){
						form_admin2.t_alamat.focus();	
						return false;
				  }
				  
				  if(status == ""){
						form_admin2.cmb_status.focus();	
						return false;
				  }
                    ...
                    setTimeout(function(){
					  ...
					  //jQuery('#btn_submit_admin').hide();
						// $("#btn_submit_admin").modal('hide'); 
						  $("#myModal").modal('hide'); 
						  window.location = base_url+'/viewdata/pendonor/';
						//load_admin("admin");
                    }, 2000);
